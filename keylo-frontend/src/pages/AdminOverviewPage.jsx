...
export default function AdminOverviewPage() {
  ...
  return <div className="bg-surface min-h-screen font-body-md text-on-surface">
    ...
    <section className="grid grid-cols-2 lg:grid-cols-6 gap-md mb-xl">
      {[
      ...
      ].map(([label, value, icon, color]) => <article key={label} className={`${color} border-2 border-primary p-md shadow-[4px_4px_0px_0px_#000000]`}><span className="material-symbols-outlined text-primary">{icon}</span><p className="font-price-display text-[26px] text-primary mt-sm">{value}</p><p className="font-label-caps text-[10px] text-primary uppercase mt-xs">{label}</p></article>)}
    </section>

    <section className="grid grid-cols-1 xl:grid-cols-2 gap-xl mb-xl">
      <div><div className="flex items-end justify-between mb-md"><div><p className="font-label-caps text-label-caps text-electric-purple uppercase">People</p><h2 className="font-h3 text-h3 text-primary">Tenants</h2></div><span className="font-label-caps text-label-caps text-on-surface-variant">{tenants.length} total</span></div><Table headers={['Tenant', 'Email', 'Joined', 'Verified']}>
      ...
      <div><div className="flex items-end justify-between mb-md"><div><p className="font-label-caps text-label-caps text-electric-purple uppercase">People</p><h2 className="font-h3 text-h3 text-primary">Landlords</h2></div><span className="font-label-caps text-label-caps text-on-surface-variant">{landlords.length} total</span></div><Table headers={['Landlord', 'Email', 'Listings', 'Verified']}>
    ...
    <section className="grid grid-cols-1 xl:grid-cols-2 gap-xl mb-xl"><div><div className="mb-md"><p className="font-label-caps text-label-caps text-electric-purple uppercase">Rent Essentials</p><h2 className="font-h3 text-h3 text-primary">Rental bookings</h2></div><Table headers={['Tenant', 'Item', 'Status', 'Dates', 'Total']}>
    ...
    </Table></div><div><div className="mb-md"><p className="font-label-caps text-label-caps text-hot-pink uppercase">Trust & safety</p><h2 className="font-h3 text-h3 text-primary">Open disputes</h2></div><Table headers={['Tenant', 'Property', 'Status', 'Claimed', 'Opened']}>
    ...
    <section className="grid grid-cols-1 xl:grid-cols-2 gap-xl"><div><div className="mb-md"><p className="font-label-caps text-label-caps text-electric-purple uppercase">Money movement</p><h2 className="font-h3 text-h3 text-primary">Recent payments</h2></div><Table headers={['Payer', 'Type', 'Status', 'Amount', 'Paid at']}>
    ...
    </Table></div><div><div className="mb-md"><p className="font-label-caps text-label-caps text-electric-purple uppercase">Latest accounts</p><h2 className="font-h3 text-h3 text-primary">Recent users</h2></div><Table headers={['Name', 'Role', 'Email', 'Last sign-in']}>
