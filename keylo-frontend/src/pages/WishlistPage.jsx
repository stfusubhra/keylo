...
export default function WishlistPage() {
  ...
  return <div className="w-full">
    ...
    <section><div className="flex items-end justify-between mb-md"><div><p className="font-label-caps text-label-caps text-hot-pink uppercase">Rent Essentials</p><h2 className="font-h3 text-h3 text-primary">Saved rentals</h2></div><span className="font-label-caps text-label-caps text-on-surface-variant">{savedRentals.length}</span></div>
      {savedRentals.length ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">{savedRentals.map(({ item, created_at: createdAt }) => <article key={item.id} className="border-2 border-primary bg-surface-container-lowest shadow-[4px_4px_0px_0px_#000000] overflow-hidden"><div className="relative"><img loading="lazy" src={item.useImage ? item.image : categoryImages[item.category] || item.image} alt={item.name} className="w-full h-44 object-cover border-b-2 border-primary" /><button type="button" onClick={() => removeRental(item.id)} aria-label={`Remove ${item.name} from wishlist`} className="absolute top-md right-md p-xs border-2 border-primary bg-hot-pink text-white"><span className="material-symbols-outlined" style={{ fontVariationSettings: 'FILL 1' }}>favorite</span></button></div><div className="p-lg"><p className="font-label-caps text-label-caps text-electric-purple uppercase">{item.categoryLabel}</p><h3 className="font-h3 text-h3 text-primary mt-xs">{item.name}</h3><p className="font-price-display text-price-display text-primary mt-md">{item.price} <span className="font-body-md text-on-surface-variant">{item.period}</span></p><div className="flex gap-sm mt-md"><Link to={`/rentals/rent/${item.id}`} className="flex-1 text-center px-md py-sm bg-primary text-on-primary border-2 border-primary font-label-caps text-label-caps">VIEW ITEM</Link><span className="px-md py-sm border-2 border-primary font-label-caps text-[10px] text-on-surface-variant">Saved {formatDate(createdAt)}</span></div></div></article>)}</div> : <EmptyState icon="🛏️" title="No saved rentals yet" description="Browse Rent Essentials to start saving." action={<Link to="/rentals" className="inline-block px-lg py-md bg-acid-lime border-2 border-primary font-label-caps text-label-caps text-primary hover:bg-primary hover:text-on-primary transition-all duration-200">+ SAVE A RENTAL</Link>} />}
    </section>
  </div>;
}
