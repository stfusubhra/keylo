...
export default function DepositDisputesPage() {
  ...
  const submit = async (event) => {
    ...
    setError('');
    try {
      await openDepositDispute({ bookingId, reason });
      setReason('');
      setBookingId('');
  ...
  return <div className="bg-surface min-h-screen font-body-md text-on-surface p-lg lg:p-xl"><div className="max-w-5xl mx-auto">
    ...
    <section className="bg-surface-container-lowest border-2 border-primary p-lg mb-xl"><h2 className="font-h3 text-h3 text-primary mb-md">Open a dispute</h2><form className="grid gap-md" onSubmit={submit}><label className="font-label-caps text-label-caps">Booking<select required value={bookingId} onChange={(event) => setBookingId(event.target.value)} className="block w-full mt-xs border-2 border-primary p-md bg-surface text-primary"><option value="">Choose a confirmed stay</option>{bookings.map((booking) => <option key={booking.id} value={booking.id}>{booking.properties?.name || 'KeyLo stay'} · Deposit ₹{Number(booking.deposit_amount).toLocaleString('en-IN')}</option>)}</select></label><label className="font-label-caps text-label-caps">What happened<textarea required minLength={10} value={reason} onChange={(event) => setReason(event.target.value)} className="block w-full mt-xs border-2 border-primary p-md bg-surface text-primary" rows="4" placeholder="Describe the condition and attach evidence in your handover record." /></label><button className="w-full sm:w-fit px-lg py-md bg-acid-lime border-2 border-primary font-label-caps text-label-caps text-primary" type="submit">OPEN DISPUTE</button></form></section>
